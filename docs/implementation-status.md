...
- Make Product Shell Actually Useful (T021): Fixed critical evidence source_type mismatch (`concept_occurrence` / `rtl_source` vs old `p1b_concept` / `p1b_rtl`) across `overview_models.py`, `page_view_models.py`, `trace_view_models.py`. Rewrote `current_understanding` to include L5/L6 files/symbols, RTL modules/signals, mapping claim details with confidence/bridge_kind, why-not-confirmed explanation. Added metric card descriptions. Added Agent QA page header note ("本地确定性 Agent，不调用 LLM"). Enhanced evidence page with group descriptions and empty-state messages. Fixed top bar to show current artifact path. Enhanced Agent Runtime page to detect and display noop_run path. Added `tests/test_agent_panel.py` with 10 tests covering all 9 question types + unsupported fallback. 14 new tests total. 454 tests pass. 0 basedpyright errors.
- Graph-first Concept Understanding + Agent Query Fix (T022): Fixed Chinese suggested question routing in `agent_panel_models.py` by reordering keyword checks (summary first, evidence before claims, edges before nodes) and adding keywords "整体情况", "关系". Added `concept_graph_view.py` with `ConceptGraphViewModel`, `ConceptGraphScene` (QGraphicsScene), and `build_concept_graph_view_model()` — deterministic three-column layout (L5/L6 → mapping → RTL), colored ellipses by node kind, edge styles by confidence, click-to-show-details. Integrated graph view into concept trace page (graph first, tables later). Added evidence strength explanation header to evidence page (strong/medium/weak/unknown meanings + "why important"). Rewrote all 9 agent answer builders with Chinese natural language templates. Added `tests/test_concept_graph_view.py` with 9 tests. 9 new tests total. 463 tests pass. compileall clean.
- Concept Auto-Discovery (T035): `discover_concepts()` scans L5/L6 Python AST + RTL regex + test files to extract symbol candidates. Scoring with cross-stage bonus, domain term bonus, generic penalty. `select_for_trace()` picks top N. `GENERIC_STOP_WORDS` filters noise. `--concepts auto` support in `p1b-trace-project`.
- Discovery Quality + Golden Spec (T036/T037): `evaluate_discovery()` compares selected concepts against golden benchmark spec. Produces precision-like / recall-like metrics. `discovery_eval_result.json` + `discovery_eval_report.md`. Calibrated auto-trace with canonicalization and filtering. T037 test covers precision/recall gates, excluded terms, eval schema.
- Semantic Pipeline View (T039/T040): `build_semantic_pipeline_view()` creates lane-based pipeline view (L5 → L6 → RTL → Tests) with cross-stage edges enriched by reason, evidence_ids, source_files. `semantic_pipeline_view.json` schema. Pipeline UI in Tauri with React Flow. Agent Q&A routing for pipeline/dataflow questions.
- Agent Navigation Index (T041): `build_agent_navigation_index()` produces deterministic navigation artifact from all existing artifacts. Entrypoints, question_routes, concept_routes, edge_routes, quality_status, limitations. Integrated into CLI pipeline. Rust loader + TypeScript types. Agent Q&A enhanced with navigation-aware answers (从哪里开始, 下一步, 噪声概念, 完整证据链, edge证据). T041.1 hardened: pipeline_view initialization robustness, source_provenance schema alignment, bundle test skip-on-missing, TypeScript tests (53 passed), quality route prefers agent_navigation_index.quality_status over raw eval.
- Agent Provider Boundary (T042): Three provider types (`deterministic`, `offline_mock`, `external_disabled`) with `AgentRunResult` / `AgentRunTrace` contract. Deterministic provider wraps existing `answerQuestion()` with trace generation. Offline Mock provider simulates multi-step agent navigation using local artifacts only — no network, no API key. External Disabled provider is a stub returning a disabled message. Unified `runAgent()` dispatcher. AgentQA page features provider selection UI (Deterministic / Offline Mock / External Disabled), trace display with steps/artifacts/evidence/limitations, and provider badge per history entry. 15 new TypeScript tests (68 total pass). All providers set `external_calls_made: false`.
- Provider Safety Gate (T043/T043.1): Policy layer (`evaluateProviderPolicy()`) gates all provider runs before execution. Deterministic/offline_mock allowed; external_disabled/unknown denied. Config schema (`ProviderConfig`) forbids api_key/token/secret/endpoint/base_url/headers fields — `validateProviderConfig()` rejects any secret-containing config. Audit event (`ProviderRunAuditEvent`) recorded for every run with question_preview (max 80 chars), policy decision, trace_id, artifacts_used — in-memory only, no disk write, no API key. AgentQA displays policy status line (allowed/denied, network, external_calls, secret_storage) and audit summary per answer card. **T043.1 hardened**: audit `question_preview` redaction for api_key/token/sk-/ghp_/xoxb-/JWT patterns — returns `[redacted sensitive-looking question]`. Canonical `policy_result` and `audit_event` centralized in `runAgent()` — provider internal fallbacks overwritten. Unknown provider denied decision preserved in `policy_result.provider_kind`. Provider direct calls (`.run()`) do not append global audit log. 47 new TypeScript tests (115 total pass). All providers still `external_calls_made: false`, no network calls, no secret storage. Policy version "t043.0".
- LLM Context Builder + Dry-run External Request Plan (T044/T044.1): Offline context assembly layer answering "what bounded context would an external provider receive?" Three new modules: `sourcePack.ts` (`buildSourceEvidencePack()`) collects concept/edge/node evidence from nav index, pipeline view, graph; `contextBuilder.ts` (`buildLlmContext()`) assembles bounded context with ≤20 items, ≤500 char previews, rough token estimates (chars/4); `requestPlan.ts` (`buildDryRunExternalRequestPlan()`) produces a blocked preview (`dry_run: true`, `policy_allowed: false`) with hypothetical system/user prompt previews. No API calls, no secrets, no network. AgentQA result cards feature "上下文预览 (Dry-run)" toggle showing context items, artifacts, token estimate, and prompt previews. **T044.1 hardened**: `buildHypotheticalUserPrompt()` no longer accepts raw question — uses redacted `contextBundle.question_preview` only. `JSON.stringify(plan)` guaranteed free of sensitive raw question. sourcePack fallback limitation distinguishes "no nav" vs "nav exists but no match". 42 new TypeScript tests (157 total pass). All T043 security guarantees maintained: no API keys, no network, no secrets.
- Approval-Gated External Provider Runtime Shell (T045): Runtime shell for future external LLM provider with manual approval gate. `externalRequestPackage.ts` (`buildExternalRequestPackage()`) builds standardized request package with `send_allowed=false`, `approval_required=true`, `risk_summary` (raw_question_included=false, api_key_required=false, network_call_planned=false, secret_storage_planned=false, external_call_blocked_by_policy=true). `approvalGate.ts` state machine with states `not_requested` → `previewed` → `approved_but_blocked` | `denied` — all states block network send. AgentQA adds "T045 请求包预览" toggle with request metadata, risk summary, messages preview, approval decision display, and Simulate Manual Approval / Deny buttons. Even approved requests show "approved_but_blocked" with explicit "Network send remains blocked in T045" warning. 36 new TypeScript tests (193 total pass). No real external API calls, no API key storage, no network send.
- External Provider Execution Pipeline (T046): Full engineering skeleton for future external provider adapters — still no real network calls. `externalProviderRegistry.ts` catalogs all providers (external_disabled, mock_external_llm, future_openai/anthropic/deepseek/gemini/glm) with `send_allowed=false`, `network_allowed=false`, `secret_storage_allowed=false`. `egressGuard.ts` is the final network-send gate — always `allowed=false`, with state-specific reasons (approved_but_blocked → "blocked before network in T046", future → "placeholder only"). `externalTransport.ts` provides blocked transport (no mock answer) and mock transport (deterministic offline answer preview). `externalExecution.ts` is the end-to-end pipeline shell: `buildExternalRequestPackage()` → approval gate → transport → `ExternalExecutionResult`. AgentQA adds "T046 执行管线" toggle with provider selector, transport selector, execution result display, egress guard reason, mock answer preview, and Preview only / Simulate approval / Deny buttons. 42 new TypeScript tests (235 total pass). No real external API calls, no API key storage, no network send, no endpoint configuration.
- Ephemeral Real Provider Adapter + Manual Send Gate (T047): First real external LLM provider capability with strict security. `ephemeralProviderSession.ts` manages memory-only session with key fingerprint (not raw key) and endpoint origin preview. `realProviderContract.ts` defines frontend-backend invocation request/result with NO api_key field in TS contract. `realProviderAudit.ts` records redacted audit summaries in memory only — no raw key, prompt, or response ever stored. Rust backend `provider_runtime.rs` provides `invoke_openai_compatible_ephemeral` command with 60s timeout, HTTPS-only endpoint validation, error redaction, and response preview truncation. `evaluateRealSendGate()` enforces 5 conditions (session configured, key present, HTTPS endpoint, approval adequate, explicit user consent) — all must be true for `allowed=true`. T046 `evaluateEgressGuard()` remains UNCHANGED (always `allowed=false`). AgentQA adds "T047 真实 Provider" toggle with session configuration panel, fingerprint display, send gate status, explicit confirmation dialog, and result display. 37 new TypeScript tests (272 total pass). 12 Rust tests pass. Security scan: zero hardcoded keys. Production build clean.

**T047.1 Security Hardening** (PR #4 patch):
- **Rust error preview redaction**: Non-2xx provider responses no longer return raw body. `build_provider_error_preview()` redacts then truncates to 500 chars max. `redact_error()` covers sk-/ghp_/xoxb-/Bearer/Authorization/api_key=/token=/password=/JWT eyJ... patterns.
- **Char-safe truncation**: `truncate_chars()` uses Unicode char iteration instead of byte slicing — safe for Chinese and emoji.
- **Backend secondary gate**: `invoke_openai_compatible_ephemeral` now receives `provider_id`, `approval_state`, `send_allowed_by_user` and validates all three before any network call. `validate_backend_gate()` blocks: invalid provider_id, wrong approval_state, missing consent, empty API key.
- **Endpoint hardening**: `validate_endpoint_url()` uses `url::Url` parsing, rejects private IPv4 (10.x, 172.16-31.x, 192.168.x, 127.x, 0.0.0.0), blocks query params containing api_key/key/token/password/secret.
- **Provider ID gate**: `evaluateRealSendGate()` adds `provider_allowed` condition — only `openai_compatible_ephemeral` accepted. 6 conditions total, all must be true.
- **UI consent fix**: `evaluateT047SendGate()` takes explicit `sendAllowedByUser` parameter. Session config and panel open always pass `false`. Only "确认发送一次" passes `true` after confirmation dialog. Consent checkbox ✗ before confirmation, ✓ only after.
- **Frontend error redaction**: `redactUiErrorPreview()` in `realProviderContract.ts` covers sk-/ghp_/xoxb-/Bearer/Authorization/api_key/token/password with 1000-char cap. AgentQA catch block uses it instead of `String(err)`.
- **Tests**: 289 TS tests pass (17 files), 47 Rust tests pass (12 artifact_loader + 35 provider_runtime). 8 new egress-guard tests, 9 new real-provider-contract tests, 35 new Rust provider_runtime tests.
