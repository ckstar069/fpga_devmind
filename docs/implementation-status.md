...
- LLM Context Builder + Dry-run External Request Plan (T044): Offline context assembly layer answering "what bounded context would an external provider receive?" Three new modules: `sourcePack.ts` (`buildSourceEvidencePack()`) collects concept/edge/node evidence from nav index, pipeline view, graph; `contextBuilder.ts` (`buildLlmContext()`) assembles bounded context with ≤20 items, ≤500 char previews, rough token estimates (chars/4); `requestPlan.ts` (`buildDryRunExternalRequestPlan()`) produces a blocked preview (`dry_run: true`, `policy_allowed: false`) with hypothetical system/user prompt previews. No API calls, no secrets, no network. AgentQA result cards feature "上下文预览 (Dry-run)" toggle showing context items, artifacts, token estimate, and prompt previews. 35 new TypeScript tests (150 total pass). All T043 security guarantees maintained: no API keys, no network, no secrets.
