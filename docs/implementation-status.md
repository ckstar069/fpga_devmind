...
- Provider Safety Gate (T043): Policy layer (`evaluateProviderPolicy()`) gates all provider runs before execution. Deterministic/offline_mock allowed; external_disabled/unknown denied. Config schema (`ProviderConfig`) forbids api_key/token/secret/endpoint/base_url/headers fields — `validateProviderConfig()` rejects any secret-containing config. Audit event (`ProviderRunAuditEvent`) recorded for every run with question_preview (max 80 chars), policy decision, trace_id, artifacts_used — in-memory only, no disk write, no API key. AgentQA displays policy status line (allowed/denied, network, external_calls, secret_storage) and audit summary per answer card. 36 new TypeScript tests (104 total pass). All providers still `external_calls_made: false`, no network calls, no secret storage. Policy version "t043.0".
